...
public class UpdateHandler(ITelegramBotClient bot, ILogger<UpdateHandler> logger, IYaPrompter promter) : IUpdateHandler
{
    ...
    async Task<Message> SendAlert(Message msg)
    {
        ...
        var response = await promter.SendAsync(prompt) ?? "Алиса сдохла";
